write_econdata <- function(db, agencyid, id, version, provideragencyid, providerid, ...) 
{
  ...
  for( index in 1:length(names(db)) ) {
    ...
    dataset$DataSets[[1]]$Series[[index]] <- lapply(attributes(db[[series]])$metadata, unbox)
    dataset$DataSets[[1]]$Series[[index]]$Obs <- data.frame(db[[series]], TIME_PERIOD = time_period)
  }
  
  data_message <- toJSON(dataset)
